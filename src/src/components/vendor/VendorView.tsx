...
import { Truck, Plus, Phone, Mail, MapPin, Star, Calendar, FileText, X, Pencil, Trash2 } from 'lucide-react';
import { Vendor } from '../../types/inventory';

interface VendorViewProps {
  vendors: Vendor[];
  onAddVendor: (v: Partial<Vendor>) => void;
  onUpdateVendor?: (id: string, v: Partial<Vendor>) => void;
  onDeleteVendor?: (id: string) => void;
}

export const VendorView: React.FC<VendorViewProps> = ({ vendors, onAddVendor, onUpdateVendor, onDeleteVendor }) => {
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus vendor ini?')) {
      if (onDeleteVendor) onDeleteVendor(id);
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingVendor) {
      if (onUpdateVendor) onUpdateVendor(editingVendor.id, formData);
    } else {
