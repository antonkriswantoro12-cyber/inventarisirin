...
export const PreventiveMaintenanceView: React.FC<PMViewProps> = ({
  ...
  const [editingPM, setEditingPM] = useState<PMRecord | null>(null);
  ...
  const handleOpenEdit = (pm: PMRecord) => {
    setEditingPM(pm);
    setFormData(pm);
    setIsModalOpen(true);
  };
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus catatan PM ini?')) {
      if (onDeletePM) onDeletePM(id);
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingPM) {
      onUpdatePM(editingPM.id, formData);
    } else {
      onAddPM(formData);
    }
        ...
        {pms.map((pm) => (
          <div key={pm.id} className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-2xs hover:shadow-md transition-shadow">
            <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-3 gap-2">
              ...
              <div className="flex items-center space-x-2 text-left md:text-right text-xs">
                <div>
