...
export function App() {
  ...
  const [currentUser] = useState('Anton Kriswantoro, S.ST');
  ...
  const handleAddLoan = (loan: Partial<EquipmentLoan>) => {
    const newLoan: EquipmentLoan = {
      id: `loan-${Date.now()}`,
      equipmentId: loan.equipmentId || equipment[0].id,
      equipmentName: loan.equipmentName || equipment[0].name,
      inventoryNo: loan.inventoryNo || equipment[0].inventoryNo,
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          ...
          {activeTab === 'room' && (
            <RoomView rooms={rooms} equipment={equipment} />
          )}
